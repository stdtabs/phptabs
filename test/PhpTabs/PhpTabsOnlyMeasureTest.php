...
/**
 * Reduce one song to particular measure for all track
 */
class PhpTabsOnlyMeasureTest extends TestCase
{
    /**
     ...
     */
    public function testOnlyOneMeasure($song, $measureIndex)
    {
        ...
        // Measure tempo
        foreach ($onlySong->getTracks() as $track) {
            $this->assertEquals(
                1,
                $track->countMeasures()
            );
            ...
            $this->assertEquals(
                $expectedTempo,
                $track->getMeasure(0)->getTempo()->getValue()
            );
        }
    }
}
